...
class AuthService:
    ...
    def _send_reset_email(cls, email: str, username: str, token: str):
        ...
        html = f"<p>Hello <strong>{username}</strong>,</p><p><a href='{reset_url}'>Reset Password</a></p>"
        cls._send_email(email, 'Applyce Password Reset', text, html)

    @classmethod
    def _send_welcome_email(cls, recipient_email: str, username: str):
        text = f"Hi {username}, welcome to Applyce!"
        html = f"<p>Hi <strong>{username}</strong>, welcome to Applyce!</p>"
        cls._send_email(recipient_email, 'Welcome to Applyce', text, html)
    ...
    def get_or_create_oauth_user(cls, provider: str, provider_user_id: str, email: str, username_hint: str):
        linked = OAuthAccount.query.filter_by(provider=provider, provider_user_id=str(provider_user_id)).first()
        if linked:
            return linked.user

        email_norm = cls.normalize_email(email)
        user = User.query.filter(func.lower(User.email) == email_norm).first() if email_norm else None

        if not user:
            base = cls.normalize_username(username_hint or email_norm.split('@')[0] if email_norm else 'github_user')
            if not base:
                base = 'github_user'
            candidate = base
            idx = 1
            ...
            user = User(username=candidate, email=email_norm or f"{candidate}@users.noreply.github.com", is_active=True)
            user.password_hash = None
            db.session.add(user)
            db.session.commit()

        return cls.link_oauth_account(user, provider, str(provider_user_id), provider_email=email_norm)
